...
var removed = InitializeGraph();

bool[][] InitializeGraph() => graph.Select(line => line.Select(_ => false).ToArray()).ToArray();
int sum = 0;
int localMinimalSum = 0;
// O(c*n*m)
while (true) // O(c) total number of times a removal can occur
{
